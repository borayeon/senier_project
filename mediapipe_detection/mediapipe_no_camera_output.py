...
# 출력부 초기 상태 정의
EAR_status = "Awake"  # 기본 상태
drowsy0_status = ""  # 기본 상태는 '하품이 감지되지 않음'
drowsy1_status = ""
drowsy2_status = ""
drowsy3_status = ""
# 트리거
...
def detect_yawning(face_landmarks, image_shape, yawn_start_time):
    global drowsy0_status, yawn_duration, drowsy_0
    # 상입술과 하입술의 랜드마크 인덱스
    ...
    if lip_distance > yawn_threshold:
        if yawn_start_time == 0:  # 하품이 시작된 시간이 기록되지 않았다면
            ...
            if yawn_duration >= 3:  # 하품이 3초 이상 지속되었다면
                drowsy0_status = "Yawn Detected"
                drowsy_0 = True
    else:
        yawn_start_time = 0  # 하품이 감지되지 않으면 시작 시간을 리셋
        yawn_duration = 0  # 지속 시간도 리셋
        drowsy0_status = "No Yawn Detected"
        drowsy_0 = False

    return drowsy_0, drowsy0_status, yawn_duration, yawn_start_time
# 입 크기 계산
...
def check_drowsiness(closing_durations):
    return len([d for d in closing_durations if d >= 0.5]) >= 3
# 눈 지속 시간 계산
def calculate_eye_closing_time(ear):
    global blink_timestamp, closing_durations, blink_count, drowsy_1
    current_time = time.time()
    # 눈의 개방 정도(ear)가 임계값(EAR_THRESHOLD)보다 작고, 눈을 감기 시작한 시간이 기록되지 않았다면
    if ear < threshold.EAR_THRESHOLD and blink_timestamp == 0:
    ...
    # 눈의 개방 정도가 임계값 이상이고, 눈을 감기 시작한 시간이 기록되어 있다면 (즉, 눈이 다시 열렸다면)
    elif ear >= threshold.EAR_THRESHOLD and blink_timestamp != 0:
        ...
        drowsy_1 = check_drowsiness(closing_durations)  # 눈 깜박임이 3회 이상이면 true 반환
        return True
...
def calculate_blink_count_and_rate():
    ...
    if elapsed_time >= 60:
        blink_rate = blink_count  # 분당 깜박임 수
...
def calculate_perclos():
    ...
    current_time = time.time()
    if current_time - start_measurement_time >= 60:  # 60초마다 PERCLOS 계산
...
tmp = list(drowsy_0,drowsy_1,drowsy_2,drowsy_3)
...
#----------------------------------------------------------------------------------------
                # # 하품
                # # 입 크기 계산 - 실시간 업데이트
                drowsy_0, drowsy0_status, yawn_start_time, yawn_duration = detect_yawning(face_landmarks, image.shape, yawn_start_time)
#----------------------------------------------------------------------------------------
                # 눈 깜빡임 지속시간 - 실시간 업데이트
                # ear 계산
                ear = EAR_calculation(face_landmarks, image.shape)
                # 눈 깜빡임 시 눈 감김 지속 시간이 500ms을 넘으면 상태 True로 변환
                _ = calculate_eye_closing_time(ear)
#----------------------------------------------------------------------------------------
...
# 4
                ...
                # EAR 기반 졸음 감지
                if ear > threshold.EAR_THRESHOLD:
                    EAR_status = "EAR : Awake"
                else:
                    EAR_status = "EAR : Sleepy"

                # 눈 감김 지속시간
                if drowsy_1:
                    drowsy1_status = "ECT : drowsy"
                else:
                    drowsy1_status = "ECT : not drowsy"

                # 분당 눈 깜빡이
                if drowsy_2:
                    drowsy2_status = "PER_NUM : drowsy"
                else:
                    drowsy2_status = "PER_NUM : not drowsy"

                # perclos
                if drowsy_3:
                    drowsy3_status = "perclos : drowsy"
                else:
                    drowsy3_status = "perclos : not drowsy"

                #print(drowsy_0,drowsy_1,drowsy_2,drowsy_3)
                # # head
                # if head_down_count:
                #     head_status = "head on"
                # else:
        ...
        # EAR
        cv2.putText(flipped_image, EAR_status, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2,
                    cv2.LINE_AA)
        # 졸음 감지 상태 표시
        # 하품 감지 상태를 좌우 반전된 영상에 표시
        cv2.putText(flipped_image, drowsy0_status, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2, cv2.LINE_AA)
        #cv2.putText(flipped_image, ear_text, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2, cv2.LINE_AA)
        # 눈 지속시간
        cv2.putText(flipped_image, drowsy1_status, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2, cv2.LINE_AA)
        # 분당 눈 깜빡임
        cv2.putText(flipped_image, drowsy2_status, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2, cv2.LINE_AA)
        # perclos
        cv2.putText(flipped_image, drowsy3_status, (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2, cv2.LINE_AA)
        # head
